...
def test_daily_integration_failure_emits_artifact_integrity_and_no_partial_outputs(tmp_path: Path) -> None:
    ...
    assert not (reports_root / "merchant_daily_snapshot.json").exists()
    assert not (reports_root / "merchant_daily_snapshot.md").exists()
    assert not (reports_root / "merchant_daily_snapshot.pdf").exists()
