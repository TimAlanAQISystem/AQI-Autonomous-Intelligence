...
# ============================================================================
# CALL SESSION FSM — Phone Call Lifecycle State Machine
# ============================================================================
# Phase 2 Restoration: Replaces scattered boolean flags with a single
# deterministic state machine.
#
# "Boolean flags leak, contradict each other, create ghost states, cause drift
# under stress, and cannot be audited. The FSM solves all of that in one
# stroke." — Tim (Founder)
#
# Replaces these flags:
#   context['stream_started']        → fsm.is_stream_ready
#   context['stream_ended']          → fsm.has_ended
#   context['greeting_sent']         → fsm.greeting_sent
#   context['first_turn_complete']   → fsm.in_dialogue
#   context['conversation_state']    → fsm.conversation_state_str
#   context['fallback_suppressed_until'] → fsm.pitch_suppressed
#
# Backward compatibility: _sync_context() updates old flags on every
# transition so existing code that reads them directly still works.
# ============================================================================


class CallFlowState(Enum):
    """States of a phone call lifecycle"""
...
class CallSessionFSM:
    """
    Phone call lifecycle state machine.

    Replaces 6 scattered boolean/string flags with a single deterministic
    state machine that:
      1. Transitions only through events (not direct state assignment)
      2. Logs every transition for governance audit
      3. Maintains backward-compatible context flags via _sync_context()
      4. Cannot enter ghost states (contradictory flag combinations impossible)
    ...
    # Pitch suppression window after greeting (seconds)
    GREETING_SUPPRESSION_SECONDS = 20.0

    def __init__(self, context: dict, call_sid: str = ""):
        self.state = CallFlowState.INIT
        self.context = context
        self.call_sid = call_sid
        self.transitions: list = []
        self.end_reason: str = ""
        self._state_entered_at: float = time.time()
        self._greeting_suppressed_until: float = 0.0
        self._sync_context()

    def handle_event(self, event: CallFlowEvent, metadata: dict = None) -> bool:
        """
        Process event and transition if valid.
        Returns True if a transition occurred, False if event was invalid/ignored.
        Idempotent: repeated events that don't change state return False silently.
        """
        old_state = self.state
        new_state = None

        # ── Transition rules ──────────────────────────────────────────
        if self.state == CallFlowState.INIT:
            if event == CallFlowEvent.STREAM_START:
                new_state = CallFlowState.STREAM_READY
            elif event == CallFlowEvent.CALL_END:
                new_state = CallFlowState.ENDED

        elif self.state == CallFlowState.STREAM_READY:
            if event == CallFlowEvent.GREETING_BUILT:
                new_state = CallFlowState.GREETING_PENDING
            elif event == CallFlowEvent.FAST_START:
        ...
        elif self.state == CallFlowState.GREETING_PLAYED:
            if event == CallFlowEvent.FIRST_SPEECH:
                new_state = CallFlowState.DIALOGUE
            elif event == CallFlowEvent.CALL_END:
                new_state = CallFlowState.ENDED

        elif self.state == CallFlowState.DIALOGUE:
            if event == CallFlowEvent.CALL_END:
                new_state = CallFlowState.ENDED

        elif self.state == CallFlowState.ENDED:
        ...
        if new_state and new_state != old_state:
            ...
            if event == CallFlowEvent.CALL_END and metadata:
                self.end_reason = metadata.get('reason', self.end_reason)
            self._log_transition(old_state, new_state, event, metadata)
            self._sync_context()
            return True

        return False

    def end_call(self, reason: str = "") -> bool:
        """Convenience method for ending the call from any state."""
        return self.handle_event(CallFlowEvent.CALL_END, {'reason': reason})

    # ── Query Properties (replace boolean flag reads) ─────────────────

    @property
    def has_ended(self) -> bool:
        """Replaces: context.get('stream_ended')"""
        return self.state == CallFlowState.ENDED

    @property
    def is_stream_ready(self) -> bool:
        """Replaces: context.get('stream_started')"""
        return self.state not in (CallFlowState.INIT, CallFlowState.ENDED)

    @property
    def greeting_sent(self) -> bool:
    ...
    def in_dialogue(self) -> bool:
        """Replaces: context.get('first_turn_complete') and conversation_state == 'dialogue'"""
        return self.state == CallFlowState.DIALOGUE
    ...
    @property
    def pitch_suppressed(self) -> bool:
        """Replaces: time.time() < context.get('fallback_suppressed_until', 0)"""
        return time.time() < self._greeting_suppressed_until

    @property
    def conversation_state_str(self) -> str:
        """Backward-compatible string for context['conversation_state']."""
        _map = {
        ...
        return _map.get(self.state, 'greeting')

    # ── Backward Compatibility ────────────────────────────────────────

    def _sync_context(self):
        """
        Update old boolean/string flags in context dict so existing code
        that reads them directly continues to work unchanged.

        This is the backward-compatibility bridge. As flag reads are
        incrementally migrated to FSM properties, this method can shrink.
        """
        self.context['stream_ended'] = self.has_ended
        self.context['stream_started'] = self.is_stream_ready
        self.context['greeting_sent'] = self.greeting_sent
        self.context['first_turn_complete'] = self.in_dialogue
        self.context['conversation_state'] = self.conversation_state_str
        if self._greeting_suppressed_until > 0:
            self.context['fallback_suppressed_until'] = self._greeting_suppressed_until

    # ── Audit Logging ─────────────────────────────────────────────────

    def _log_transition(self, from_state, to_state, event, metadata=None):
        """Log state transition with full context for governance audit."""
        entry = {
            'timestamp': datetime.now().isoformat(),
        ...
        logger.info(
            f"[CALL FSM] {from_state.value} -> {to_state.value} "
            f"(event={event.value}, call={self.call_sid})"
        )

    def get_audit_log(self) -> list:
        """Return full transition history for governance audit."""
        return self.transitions

    def __repr__(self):
        return f"CallSessionFSM(state={self.state.value}, call={self.call_sid})"
