...
from mission_parser import MerchantWeeklyMissionParser, MissionDataValidationError
from planner import MerchantWeeklyPlanner
from verifier import MerchantWeeklyVerifier
...
def _render_markdown_report(summary: Dict[str, Any], sections: Dict[str, Any]) -> str:
    ...
    ws = period.get("week_start", "unknown")
    we = period.get("week_end", "unknown")

    lines: List[str] = []
    lines.append("# Merchant Weekly Report")
    lines.append("")
    lines.append(f"Reporting Period: {ws} to {we}")
    ...
    lines.append(f"- Chargeback Rate: {_format_pct(float(kpis.get('chargeback_rate', 0.0)))}")
    lines.append(f"- Merchant Growth Rate: {_format_pct(float(kpis.get('merchant_growth_rate', 0.0)))}")
    lines.append("")
    ...
    for item in executive.get("top_3_wins", []):
        lines.append(f"- {item}")
    lines.append("")
    lines.append("### Top Risks")
    for item in executive.get("top_3_risks", []):
        lines.append(f"- {item}")
    lines.append("")
    lines.append("### Recommended Actions")
    for item in executive.get("recommended_actions", []):
        lines.append(f"- {item}")
    lines.append("")
...
def _write_minimal_pdf(path: Path, text: str) -> None:
    # Dependency-free fallback PDF writer (single-page text output).
    safe_lines = [
...
def _build_cli() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run end-to-end weekly merchant report mission")
    parser.add_argument("--dataset", required=True, help="Input dataset path (.json, .csv, .xlsx)")
    parser.add_argument("--schema", default="missions/merchant_weekly_report/schema.json", help="Mission schema path")
    parser.add_argument("--week-start", default=None, help="Optional reporting week start YYYY-MM-DD")
    parser.add_argument("--output-dir", default="reports/merchant_weekly_report", help="Mission output directory")
...
def main() -> int:
    ...
    try:
        parser = MerchantWeeklyMissionParser(schema_path=str(schema_path), log_dir=str(output_dir))
        parsed = parser.parse(dataset_path=args.dataset, reporting_week_start=args.week_start)
        ...
        planner = MerchantWeeklyPlanner(schema_path=str(schema_path), output_dir=str(output_dir))
        ...
        verifier = MerchantWeeklyVerifier(schema_path=str(schema_path), output_dir=str(output_dir))
        ...
        if not verify_result.passed:
            _emit_final_checkpoint(
                ...
                summary_path=reports_root / "merchant_weekly_report.json",
                markdown_path=reports_root / "merchant_weekly_report.md",
                pdf_path=reports_root / "merchant_weekly_report.pdf",
            )
            _append_jsonl(
                output_dir / "mission_runs.jsonl",
                _build_run_event(
